...
private:
    ...
    void parse_spliceai_info(const std::string& info, int alt_index,
        ...
        if (fields[2] != ".") {
            ...
            try { ds_ag = std::stod(fields[2]); } catch (const std::exception&) {}
        }
        if (fields[3] != ".") {
            annotations["SpliceAI_pred_DS_AL"] = fields[3];
            try { ds_al = std::stod(fields[3]); } catch (const std::exception&) {}
        }
        if (fields[4] != ".") {
            annotations["SpliceAI_pred_DS_DG"] = fields[4];
            try { ds_dg = std::stod(fields[4]); } catch (const std::exception&) {}
        }
        if (fields[5] != ".") {
            annotations["SpliceAI_pred_DS_DL"] = fields[5];
            try { ds_dl = std::stod(fields[5]); } catch (const std::exception&) {}
        }
